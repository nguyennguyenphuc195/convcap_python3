...
def train(data_root="./data/coco/", epochs=30, batchsize=20, ncap_per_img=5, num_layers=3,\
     is_attention=True, learning_rate=5e-5, lr_step_size=15, finetune_after=8, reduce_dim=False, clip_grad=0.1, nimgfeats=4096,\
    ...
    #convcap model
    convcap_model = Convcap(train_ds.vocab_size, num_layers, is_attention, kernel_size=kernel_size, reduce_dim=reduce_dim, positional_emb=positional_emb, nimgfeats=nimgfeats)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for imgs, wordclass, mask, img_id in tqdm(train_dl):
            ...
            if is_attention:
                ...
                logits, all_attn = convcap_model(conv_feats, lin_feats, wordclass, return_all_attention=True)
                # attn = attn.view(batchsize_cap, max_tokens, height, width) #attn shape (batchsize_cap, max_tokens, height, width)
            else:
            ...
            if is_attention:
                ...
                for attn in all_attn:
                    loss += torch.sum(torch.pow(1. - torch.sum(attn, 1), 2))
                loss = loss / (batchsize_cap * height * width * len(all_attn)) + F.cross_entropy(logits[maskids, ...], wordclass[maskids, ...])
            else:
